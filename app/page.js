...
export default function Home() {
  ...
  const handleRoute = (event) => {
    ...
    setEndLocation(route.legs[0].steps[route.legs[0].steps.length - 1].maneuver.location);
    const decodedPolyline = polyline.decode(route.geometry);
    setCoords(decodedPolyline.map(point => ({
      lng: point[0],
      lat: point[1]
    })));
  };
  ...
  for (let i = 0; i <= 360; i += 10) {
    ...
    const lat = Math.asin(Math.sin(toRadians(endLocation[1])) * Math.cos(radius / earthRadius) +
      Math.cos(toRadians(endLocation[1])) * Math.sin(radius / earthRadius) * Math.cos(bearing));
    const lng = toRadians(endLocation[0]) + Math.atan2(Math.sin(bearing) * Math.sin(radius / earthRadius) * Math.cos(toRadians(endLocation[1])),
      Math.cos(radius / earthRadius) - Math.sin(toRadians(endLocation[1])) * Math.sin(lat));
    ...
    <div className="flex flex-col">
      <div className="p-2 md:pd-6 border-[2px] rounded-xl h-3/5">
        ...
        <button
          ...
          className={`p-3 ${startLocation === '' ? 'bg-gray-500' : 'bg-black'} w-full mt-5 text-white rounded-lg`}
        >
          Search
        </button>
      </div>
      {search ? (
        <div className="flex flex-col gap-y-6 py-4 justfify-center text-center items-center mx-auto">
          {CarDetails.map((item, index) => (
            <div key={index} className="flex flex-col gap-y-2 border border-gray-500 rounded-lg bg-slate-200 px-20 py-4">
